...
-- Generic sequent-calculus layer of the S4dot2 stack, parameterized over
-- a token algebra (Phase 3 of the token-algebra parameterization).  Body
-- ported verbatim from S4dot2/System.agda; the only real edits are the
-- SortedPosition-backed Syntax open replaced by the Generic one, and the
-- unsort-factor/toList-inj/discretePosition block (System.agda:208-228)
-- deleted in favour of the discretePos interface field.

open import S4dot2.TokenAlgebra.Interface

module S4dot2.Generic.System (TA : TokenAlgebra) where
...
-- ListExt is opened non-publicly by Generic.Syntax (see the note there),
-- so its names do not flow through the applied module — import directly.
...
-- Position discreteness is the token algebra's discretePos field
-- (replaces the relocated unsort-factor/toList-inj block, now in
-- S4dot2.TokenAlgebra.DiscretePosition for the NAT-SDL instance)
open TokenAlgebra TA public using () renaming (discretePos to discretePosition)
