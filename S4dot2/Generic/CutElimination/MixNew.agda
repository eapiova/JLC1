...
--
-- Generic (token-algebra-parameterized) since Phase 5 of the token-algebra
-- parameterization: body byte-identical to the pre-move
-- S4dot2/CutElimination/MixNew.agda lines 61-6813; only this header differs:
--   * module parameterized over TA; Syntax/System/Base/ProofSubst/
--     MixHelpers opens flipped to the Generic layer (ListExt opened
--     directly; the Defs surface arrives through the generic Base's
--     PUBLIC Defs re-export -- exactly as today's file takes it through
--     Base -- so the body shares Base's single module-application copy
--     of the measure heads (ProbeDelta3/4/6 receipts); discretePFormula
--     is taken from System as today and hidden from the Base open, which
--     would re-export a second copy);
--   * _<_ is an instance-overloaded comparison (ℕ measure on the right
--     argument: identity on ℕ, tokℕ on Token) so the body's mixed eigen
--     annotations "maxEigenposToken Π < x" (x : Token) elaborate to the
--     generic engine's "... < tokℕ x"; on ℕ-ℕ uses it is definitionally
--     the Nat.Order _<_, and at NAT-SDL (tokℕ = id) every use is
--     definitionally today's type;
--   * a private local module tree S4dot2.CutElimination.Base aliases the
--     generic Base's removeAll-++ so the body's 34 qualified references
--     resolve verbatim (qualified as-names are rejected by Agda 2.9).

open import S4dot2.TokenAlgebra.Interface

module S4dot2.Generic.CutElimination.MixNew (TA : TokenAlgebra) where
...
open import Cubical.Data.Nat.Order using (_≤_; ≤-refl; ≤-trans; left-≤-max; right-≤-max) renaming (_<_ to _<ℕ_)
...
open import S4dot2.Generic.System TA using (Ctx; _,,_; _⊢_; Ax; Cut; W⊢; ⊢W; C⊢; ⊢C; Exc⊢; ⊢Exc;
                                 ¬⊢; ⊢¬; ∧₁⊢; ∧₂⊢; ⊢∧; ∨⊢; ⊢∨₁; ⊢∨₂;
                                 ⇒⊢; ⊢⇒; □⊢; ⊢□; ♢⊢; ⊢♢;
                                 discretePFormula; structural)
open import S4dot2.Generic.CutElimination.Base TA hiding (Env; var; elm; solve; _++ₑ_; rem; discretePFormula)
open import S4dot2.Generic.CutElimination.ProofSubst TA using (WellFormedProof; makeWellFormed; makeWellFormed-WellFormed; δ-makeWellFormed;
...
open import S4dot2.Generic.CutElimination.MixHelpers TA using (mkSingleTokenExt;
  lemma-removeAll-cons-eq; lemma-removeAll-head-eq; lemma-removeAll-cons-neq; lemma-removeAll-head-neq)

open TokenAlgebra TA using (tokℕ)

-- Header device: measure-overloaded _<_ (identity on ℕ, tokℕ on Token)
private
  record HasTokenMeasure {ℓ} (X : Type ℓ) : Type ℓ where
    field toMeasureℕ : X → ℕ

  instance
    measureOfℕ : HasTokenMeasure ℕ
    measureOfℕ .HasTokenMeasure.toMeasureℕ n = n

    measureOfToken : HasTokenMeasure Token
    measureOfToken .HasTokenMeasure.toMeasureℕ = tokℕ

_<_ : ∀ {ℓ} {X : Type ℓ} ⦃ h : HasTokenMeasure X ⦄ → ℕ → X → Type
_<_ ⦃ h ⦄ m x = m <ℕ HasTokenMeasure.toMeasureℕ h x

-- Header device: local module tree giving the body's qualified
-- S4dot2.CutElimination.Base.removeAll-++ references their referent
-- (re-exported via open, not re-defined: defining a name already opened
-- in the enclosing scope is a ClashingDefinition)
import S4dot2.Generic.CutElimination.Base as GenericBaseUnapplied

private
  module GenericBaseApplied = GenericBaseUnapplied TA
  module S4dot2 where
    module CutElimination where
      module Base where
        open GenericBaseApplied using (removeAll-++) public
...
Mix {n = n} {Γ' = Γ'} {Δ' = Δ'} {A = A} {s = s}
    ...
    let
        ...
        d_final : δ p' ≤ max (δ (W⊢ {Γ = Γ₁} {Δ = Δ₁} {A = B} {s = t} Π)) (δ Π')
...
Mix {n = n} {Γ' = Γ'} {Δ' = Δ'} {A = A} {s = s}
    ...
    let
        ...
        d_final : δ p' ≤ max (δ (⊢W {Γ = Γ₁} {Δ = Δ_sub} {A = B} {s = t} Π)) (δ Π')
...
Mix {n = n} {Γ = Γ} {Δ = Δ} {Γ' = .(Γ₁' ,, [ B ^ t ])} {Δ' = Δ₁'} {A = A} {s = s}
    ...
    let
        ...
        d_final : δ p' ≤ max (δ Π) (δ (W⊢ {Γ = Γ₁'} {Δ = Δ₁'} {A = B} {s = t} Π'))
        d_final = subst (_≤ max (δ Π) (δ Π')) (sym d_p') dmix
    in (p' , d_final)
...
Mix {n = n} {Γ = Γ} {Δ = Δ} {Γ' = Γ₁'} {Δ' = .([ B ^ t ] ,, Δ_sub)} {A = A} {s = s}
    ...
    let
        ...
        d_final : δ p' ≤ max (δ Π) (δ (⊢W {Γ = Γ₁'} {Δ = Δ_sub} {A = B} {s = t} Π'))
        d_final = subst (_≤ max (δ Π) (δ Π')) (sym d_p') dmix
    in (p' , d_final)
...
Mix {n = n} {Γ = .(Γ1 ,, [ (B ∧ C) ^ t ])} {Δ = Δ1} {Γ' = Γ'} {Δ' = Δ'} {A = A} {s = s}
    ...
    let h-dec = mixHeight-dec-l (∧₁⊢ Π) Π Π' (height-subproof-1 Π)
        ...
        d_final : δ p' ≤ max (δ (∧₁⊢ {Γ = Γ1} {A = B} {s = t} {Δ = Δ1} {B = C} Π)) (δ Π')
        d_final = subst (_≤ max (δ Π) (δ Π')) (sym d_p') dmix
    in (p' , d_final)
...
Mix {n = n} {Γ = .(Γ1 ,, [ (C ∧ B) ^ t ])} {Δ = Δ1} {Γ' = Γ'} {Δ' = Δ'} {A = A} {s = s}
    ...
    let h-dec = mixHeight-dec-l (∧₂⊢ Π) Π Π' (height-subproof-1 Π)
        ...
        d_final : δ p' ≤ max (δ (∧₂⊢ {Γ = Γ1} {B = B} {s = t} {Δ = Δ1} {A = C} Π)) (δ Π')
        d_final = subst (_≤ max (δ Π) (δ Π')) (sym d_p') dmix
    in (p' , d_final)
  ...
  where
    ...
    handleAndCase1 accR (no neq1) _ =
      let h-dec1 = mixHeight-dec-l (⊢∧ Π1 Π2) Π1 (∧₁⊢ Π1') (height-subproof-2-l Π1 Π2)
          ...
          dΠ1≤G = left-left-≤-max
          dΠ'≤G : δ (∧₁⊢ {Γ = Γ'} {A = B'} {s = t'} {Δ = Δ''} {B = C'} Π1') ≤ GoalBound
    ...
    handleAndCase1 accR (yes eq1) (yes eq2) =
      let
        ...
        δ-res1-wf-eq : δ res1-wf ≡ δ res1
        δ-res1-wf-eq = trans (δ-makeWellFormed res1-cast) δ-res1-cast-eq
        res1-intermediate : max (δ Π1) (δ (∧₁⊢ {Γ = Γ'} {A = B'} {s = t'} {Δ = Δ''} {B = C'} Π1')) ≤ GoalBound
  ...
  where
    ...
    handleAndCase2 accR (no neq1) _ =
      let h-dec1 = mixHeight-dec-l (⊢∧ Π1 Π2) Π1 (∧₂⊢ Π1') (height-subproof-2-l Π1 Π2)
          ...
          dΠ1≤G : δ Π1 ≤ GoalBound
          dΠ1≤G = left-left-≤-max
          dΠ'≤G : δ (∧₂⊢ {Γ = Γ'} {B = B'} {s = t'} {Δ = Δ''} {A = C'} Π1') ≤ GoalBound
          dΠ'≤G = right-≤-max {δ (∧₂⊢ Π1')} {δ (⊢∧ Π1 Π2)}
          dmix1' = subst (_≤ GoalBound) (sym d_mix1_r) (≤-trans dmix1 (max-least dΠ1≤G dΠ'≤G))
          dΠ2≤G : δ Π2 ≤ GoalBound
          dΠ2≤G = right-left-≤-max
    ...
    handleAndCase2 accR (yes eq1) (yes eq2) =
      let
        ...
        δ-res1-wf-eq : δ res1-wf ≡ δ res1
        δ-res1-wf-eq = trans (δ-makeWellFormed res1-cast) δ-res1-cast-eq
        res1-intermediate : max (δ Π2) (δ (∧₂⊢ {Γ = Γ'} {B = B'} {s = t'} {Δ = Δ''} {A = C'} Π1')) ≤ GoalBound
        res1-intermediate = max-least
          (right-left-≤-max)
          (right-≤-max {δ (∧₂⊢ Π1')} {δ (⊢∧ Π1 Π2)})
        res1-bound : δ res1-wf ≤ GoalBound
  ...
  where
    ...
    handleBoxCase accR (yes eq1) (yes eq2) =
      let
        ...
        h-Π1-subst = height-subst-Δ {A = Ctx} {Γ = Γ_sub} {Q = λ D → D} {x = substCtx x t_r ([ B ^ insertToken x r ] ,, Δ_sub)} {y = [ B ^ (r ∘ t_r) ] ,, Δ_sub} succ-eq Π1_subst_step1
                    ∙ height-subst-Γ {A = Ctx} {P = λ G → G} {Δ = substCtx x t_r ([ B ^ insertToken x r ] ,, Δ_sub)} {x = substCtx x t_r Γ_sub} {y = Γ_sub} ctx-eq-l Π1_subst_raw
                    ∙ h-Π1-subst-raw
  ...
  where
    ...
    handleDiaCase accR (yes eq1) (yes eq2) =
      let
        ...
        h-Π1'-subst = height-subst-Δ {A = Ctx} {Γ = Γ'₁ ,, [ B' ^ (r' ∘ t_l) ]} {Q = λ D → D} {x = substCtx x t_l Δ'₁} {y = Δ'₁} succ-eq-raw Π1'_subst_step1
                     ∙ height-subst-Γ {A = Ctx} {P = λ G → G} {Δ = substCtx x t_l Δ'₁} {x = substCtx x t_l (Γ'₁ ,, [ B' ^ insertToken x r' ])} {y = Γ'₁ ,, [ B' ^ (r' ∘ t_l) ]} ant-eq Π1'_subst_raw
                     ∙ h-Π1'-subst-raw
  ...
  where
    handleNegAnd1 : (∀ m' → m' <Lex (n , mixHeight (⊢¬ Π_sub) Π') → Acc _<Lex_ m')
                 ...
                 → Σ (Γ_sub ,, ((Γ'₁ ,, [ (B' ∧ C') ^ r ]) - (A ^ s)) ⊢ (([ (¬ B) ^ t ] ,, Δ_sub) - (A ^ s)) ,, Δ'₁)
                     (λ p → δ p ≤ max (δ (⊢¬ Π_sub)) (δ (∧₁⊢ {Γ = Γ'₁} {A = B'} {s = r} {Δ = Δ'₁} {B = C'} Π'_sub)))
    handleNegAnd1 accR (no neq) =
      let (p_no , d_no) = handleNegNoEq degEq Π_sub Π' wfp wfp' accR neq
          ...
          d_final = subst (_≤ max (δ (⊢¬ Π_sub)) (δ (∧₁⊢ {Γ = Γ'₁} {A = B'} {s = r} {Δ = Δ'₁} {B = C'} Π'_sub))) (sym d_p') d_no
      in (p' , d_final)
    handleNegAnd1 accR (yes eq) =
      let neq_G : (A ^ s) ≢ ((B' ∧ C') ^ r)
          ...
          d_final = subst (_≤ max (δ (⊢¬ Π_sub)) (δ (∧₁⊢ {Γ = Γ'₁} {A = B'} {s = r} {Δ = Δ'₁} {B = C'} Π'_sub))) (sym d_p') dmix
      in (p' , d_final)
  ...
  where
    handleNegAnd2 : (∀ m' → m' <Lex (n , mixHeight (⊢¬ Π_sub) Π') → Acc _<Lex_ m')
                 ...
                 → Σ (Γ_sub ,, ((Γ'₁ ,, [ (C' ∧ B') ^ r ]) - (A ^ s)) ⊢ (([ (¬ B) ^ t ] ,, Δ_sub) - (A ^ s)) ,, Δ'₁)
                     (λ p → δ p ≤ max (δ (⊢¬ Π_sub)) (δ (∧₂⊢ {Γ = Γ'₁} {B = B'} {s = r} {Δ = Δ'₁} {A = C'} Π'_sub)))
    handleNegAnd2 accR (no neq) =
      let (p_no , d_no) = handleNegNoEq degEq Π_sub Π' wfp wfp' accR neq
          ...
          d_p' = subst-preserves-δ (cong (_,, Δ'₁) (sym succ-eq)) p_no
          d_final = subst (_≤ max (δ (⊢¬ Π_sub)) (δ (∧₂⊢ {Γ = Γ'₁} {B = B'} {s = r} {Δ = Δ'₁} {A = C'} Π'_sub))) (sym d_p') d_no
      in (p' , d_final)
    handleNegAnd2 accR (yes eq) =
      let neq_G : (A ^ s) ≢ ((C' ∧ B') ^ r)
          ...
          d_p' = trans (subst-preserves-δ-ctx (cong (Γ_sub ,,_) (sym ant-eq)) p_reorder)
                 (trans (structural-preserves-δ sub_left2 subset-refl p_rule) (structural-preserves-δ sub_left subset-refl mix))
          d_final : δ p' ≤ max (δ (⊢¬ Π_sub)) (δ (∧₂⊢ Π'_sub))
          d_final = subst (_≤ max (δ (⊢¬ Π_sub)) (δ (∧₂⊢ {Γ = Γ'₁} {B = B'} {s = r} {Δ = Δ'₁} {A = C'} Π'_sub))) (sym d_p') dmix
      in (p' , d_final)
  ...
  where
    handleDisj1And1 :
      ...
      → Σ (Γ_sub ,, ((Γ'₁ ,, [ (B' ∧ C') ^ r ]) - (A ^ s)) ⊢ (([ (B ∨ C) ^ t ] ,, Δ_sub) - (A ^ s)) ,, Δ'₁)
          (λ p → δ p ≤ max (δ (⊢∨₁ {Γ = Γ_sub} {A = B} {s = t} {Δ = Δ_sub} {B = C} Π_sub)) (δ (∧₁⊢ {Γ = Γ'₁} {A = B'} {s = r} {Δ = Δ'₁} {B = C'} Π'_sub)))
    handleDisj1And1 accR (no neq) =
      let (p_no , d_no) = handleDisj1NoEq degEq Π_sub Π' wfp wfp' accR neq
          ...
          p' = subst (λ D → Γ_sub ,, ((Γ'₁ ,, [ (B' ∧ C') ^ r ]) - (A ^ s)) ⊢ D ,, Δ'₁) (sym succ-eq) p_no
          d_p' = subst-preserves-δ (cong (_,, Δ'₁) (sym succ-eq)) p_no
          d_final = subst (_≤ max (δ (⊢∨₁ {Γ = Γ_sub} {A = B} {s = t} {Δ = Δ_sub} {B = C} Π_sub)) (δ (∧₁⊢ {Γ = Γ'₁} {A = B'} {s = r} {Δ = Δ'₁} {B = C'} Π'_sub))) (sym d_p') d_no
      in (p' , d_final)
    handleDisj1And1 accR (yes eq) =
      let
        ...
        d_p' = trans (subst-preserves-δ-ctx (cong (Γ_sub ,,_) (sym ant-eq)) p_reorder)
               (trans (structural-preserves-δ sub_left2 subset-refl p_rule) (structural-preserves-δ sub_left subset-refl mix))
        d_final : δ p' ≤ max (δ (⊢∨₁ {Γ = Γ_sub} {A = B} {s = t} {Δ = Δ_sub} {B = C} Π_sub)) (δ (∧₁⊢ Π'_sub))
        d_final = subst (_≤ max (δ (⊢∨₁ {Γ = Γ_sub} {A = B} {s = t} {Δ = Δ_sub} {B = C} Π_sub)) (δ (∧₁⊢ {Γ = Γ'₁} {A = B'} {s = r} {Δ = Δ'₁} {B = C'} Π'_sub))) (sym d_p') dmix
      in (p' , d_final)
  ...
  where
    handleDisj1And2 :
      ...
      → Σ (Γ_sub ,, ((Γ'₁ ,, [ (C' ∧ B') ^ r ]) - (A ^ s)) ⊢ (([ (B ∨ C) ^ t ] ,, Δ_sub) - (A ^ s)) ,, Δ'₁)
          (λ p → δ p ≤ max (δ (⊢∨₁ {Γ = Γ_sub} {A = B} {s = t} {Δ = Δ_sub} {B = C} Π_sub)) (δ (∧₂⊢ {Γ = Γ'₁} {B = B'} {s = r} {Δ = Δ'₁} {A = C'} Π'_sub)))
    handleDisj1And2 accR (no neq) =
      let (p_no , d_no) = handleDisj1NoEq degEq Π_sub Π' wfp wfp' accR neq
          ...
          p' = subst (λ D → Γ_sub ,, ((Γ'₁ ,, [ (C' ∧ B') ^ r ]) - (A ^ s)) ⊢ D ,, Δ'₁) (sym succ-eq) p_no
          d_p' = subst-preserves-δ (cong (_,, Δ'₁) (sym succ-eq)) p_no
          d_final = subst (_≤ max (δ (⊢∨₁ {Γ = Γ_sub} {A = B} {s = t} {Δ = Δ_sub} {B = C} Π_sub)) (δ (∧₂⊢ {Γ = Γ'₁} {B = B'} {s = r} {Δ = Δ'₁} {A = C'} Π'_sub))) (sym d_p') d_no
      in (p' , d_final)
    handleDisj1And2 accR (yes eq) =
      let
        ...
        d_p' = trans (subst-preserves-δ-ctx (cong (Γ_sub ,,_) (sym ant-eq)) p_reorder)
               (trans (structural-preserves-δ sub_left2 subset-refl p_rule) (structural-preserves-δ sub_left subset-refl mix))
        d_final : δ p' ≤ max (δ (⊢∨₁ {Γ = Γ_sub} {A = B} {s = t} {Δ = Δ_sub} {B = C} Π_sub)) (δ (∧₂⊢ Π'_sub))
        d_final = subst (_≤ max (δ (⊢∨₁ {Γ = Γ_sub} {A = B} {s = t} {Δ = Δ_sub} {B = C} Π_sub)) (δ (∧₂⊢ {Γ = Γ'₁} {B = B'} {s = r} {Δ = Δ'₁} {A = C'} Π'_sub))) (sym d_p') dmix
      in (p' , d_final)
...
Mix {n = n} {Γ = Γ_sub} {Δ = .([ (B ∨ C) ^ t ] ,, Δ_sub)} {Γ' = .(Γ'₁ ,, [ (B' ∧ C') ^ r ])} {Δ' = Δ'₁} {A = A} {s = s}
  ...
  where
    handleDisj2And1 :
      ...
      → Dec ((A ^ s) ≡ ((B ∨ C) ^ t))
      → Σ (Γ_sub ,, ((Γ'₁ ,, [ (B' ∧ C') ^ r ]) - (A ^ s)) ⊢ (([ (B ∨ C) ^ t ] ,, Δ_sub) - (A ^ s)) ,, Δ'₁)
          (λ p → δ p ≤ max (δ (⊢∨₂ {Γ = Γ_sub} {B = C} {s = t} {Δ = Δ_sub} {A = B} Π_sub)) (δ (∧₁⊢ {Γ = Γ'₁} {A = B'} {s = r} {Δ = Δ'₁} {B = C'} Π'_sub)))
    handleDisj2And1 accR (no neq) =
      let (p_no , d_no) = handleDisj2NoEq degEq Π_sub Π' wfp wfp' accR neq
          ...
          p' = subst (λ D → Γ_sub ,, ((Γ'₁ ,, [ (B' ∧ C') ^ r ]) - (A ^ s)) ⊢ D ,, Δ'₁) (sym succ-eq) p_no
          d_p' = subst-preserves-δ (cong (_,, Δ'₁) (sym succ-eq)) p_no
          d_final = subst (_≤ max (δ (⊢∨₂ {Γ = Γ_sub} {B = C} {s = t} {Δ = Δ_sub} {A = B} Π_sub)) (δ (∧₁⊢ {Γ = Γ'₁} {A = B'} {s = r} {Δ = Δ'₁} {B = C'} Π'_sub))) (sym d_p') d_no
      in (p' , d_final)
    handleDisj2And1 accR (yes eq) =
      let
        ...
        d_p' = trans (subst-preserves-δ-ctx (cong (Γ_sub ,,_) (sym ant-eq)) p_reorder)
               (trans (structural-preserves-δ sub_left2 subset-refl p_rule) (structural-preserves-δ sub_left subset-refl mix))
        d_final : δ p' ≤ max (δ (⊢∨₂ {Γ = Γ_sub} {B = C} {s = t} {Δ = Δ_sub} {A = B} Π_sub)) (δ (∧₁⊢ Π'_sub))
        d_final = subst (_≤ max (δ (⊢∨₂ {Γ = Γ_sub} {B = C} {s = t} {Δ = Δ_sub} {A = B} Π_sub)) (δ (∧₁⊢ {Γ = Γ'₁} {A = B'} {s = r} {Δ = Δ'₁} {B = C'} Π'_sub))) (sym d_p') dmix
      in (p' , d_final)
...
Mix {n = n} {Γ = Γ_sub} {Δ = .([ (B ∨ C) ^ t ] ,, Δ_sub)} {Γ' = .(Γ'₁ ,, [ (C' ∧ B') ^ r ])} {Δ' = Δ'₁} {A = A} {s = s}
  ...
  where
    handleDisj2And2 :
      ...
      → Dec ((A ^ s) ≡ ((B ∨ C) ^ t))
      → Σ (Γ_sub ,, ((Γ'₁ ,, [ (C' ∧ B') ^ r ]) - (A ^ s)) ⊢ (([ (B ∨ C) ^ t ] ,, Δ_sub) - (A ^ s)) ,, Δ'₁)
          (λ p → δ p ≤ max (δ (⊢∨₂ {Γ = Γ_sub} {B = C} {s = t} {Δ = Δ_sub} {A = B} Π_sub)) (δ (∧₂⊢ {Γ = Γ'₁} {B = B'} {s = r} {Δ = Δ'₁} {A = C'} Π'_sub)))
    handleDisj2And2 accR (no neq) =
      let (p_no , d_no) = handleDisj2NoEq degEq Π_sub Π' wfp wfp' accR neq
          ...
          p' = subst (λ D → Γ_sub ,, ((Γ'₁ ,, [ (C' ∧ B') ^ r ]) - (A ^ s)) ⊢ D ,, Δ'₁) (sym succ-eq) p_no
          d_p' = subst-preserves-δ (cong (_,, Δ'₁) (sym succ-eq)) p_no
          d_final = subst (_≤ max (δ (⊢∨₂ {Γ = Γ_sub} {B = C} {s = t} {Δ = Δ_sub} {A = B} Π_sub)) (δ (∧₂⊢ {Γ = Γ'₁} {B = B'} {s = r} {Δ = Δ'₁} {A = C'} Π'_sub))) (sym d_p') d_no
      in (p' , d_final)
    handleDisj2And2 accR (yes eq) =
      let
        ...
        d_p' = trans (subst-preserves-δ-ctx (cong (Γ_sub ,,_) (sym ant-eq)) p_reorder)
               (trans (structural-preserves-δ sub_left2 subset-refl p_rule) (structural-preserves-δ sub_left subset-refl mix))
        d_final : δ p' ≤ max (δ (⊢∨₂ {Γ = Γ_sub} {B = C} {s = t} {Δ = Δ_sub} {A = B} Π_sub)) (δ (∧₂⊢ Π'_sub))
        d_final = subst (_≤ max (δ (⊢∨₂ {Γ = Γ_sub} {B = C} {s = t} {Δ = Δ_sub} {A = B} Π_sub)) (δ (∧₂⊢ {Γ = Γ'₁} {B = B'} {s = r} {Δ = Δ'₁} {A = C'} Π'_sub))) (sym d_p') dmix
      in (p' , d_final)
    ...
    handleBoxAnd1 accR (yes eq) =
      let neq_G : (A ^ s) ≢ ((B' ∧ C') ^ r)
          ...
          d_p' = trans (subst-preserves-δ-ctx (cong (Γ_sub ,,_) (sym ant-eq)) p_reorder)
                 (trans (structural-preserves-δ sub_left2 subset-refl p_rule) (structural-preserves-δ sub_left subset-refl mix))
          d_final : δ p' ≤ max (δ (⊢□ ext_l freshΓ_l freshΔ_l Π_sub)) (δ (∧₁⊢ Π'_sub))
          d_final = subst (_≤ max (δ (⊢□ ext_l freshΓ_l freshΔ_l Π_sub)) (δ (∧₁⊢ {Γ = Γ'₁} {A = B'} {s = r} {Δ = Δ'₁} {B = C'} Π'_sub))) (sym d_p') dmix
      in (p' , d_final)
    ...
    handleBoxAnd2 accR (yes eq) =
      let neq_G : (A ^ s) ≢ ((C' ∧ B') ^ r)
          ...
          d_p' = trans (subst-preserves-δ-ctx (cong (Γ_sub ,,_) (sym ant-eq)) p_reorder)
                 (trans (structural-preserves-δ sub_left2 subset-refl p_rule) (structural-preserves-δ sub_left subset-refl mix))
          d_final : δ p' ≤ max (δ (⊢□ ext_l freshΓ_l freshΔ_l Π_sub)) (δ (∧₂⊢ Π'_sub))
          d_final = subst (_≤ max (δ (⊢□ ext_l freshΓ_l freshΔ_l Π_sub)) (δ (∧₂⊢ {Γ = Γ'₁} {B = B'} {s = r} {Δ = Δ'₁} {A = C'} Π'_sub))) (sym d_p') dmix
      in (p' , d_final)
