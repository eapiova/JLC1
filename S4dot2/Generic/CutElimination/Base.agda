...
-- S4dot2/CutElimination/Base.agda lines 33-859; only this header differs:
--   * module parameterized over TA; Syntax/System opens flipped to the
--     Generic layer; ListExt opened directly;
--   * the Defs open and the discretePFormula re-export stay PUBLIC, as in
--     the pre-move file.  This is load-bearing for elaboration, not just
--     surface: --lossy-unification's first-order solving only fires when
--     both sides of a constraint share the SAME module-application copy
--     of a measure head (height/δ/mixHeight...).  Consumers (MixNew,
--     CutElimination) must take the Defs names through THIS module's
--     re-export so their spellings share the copy that this module's own
--     lemma types mention (ProbeDelta3/4/6 receipts: a second, separate
--     Defs application makes the W⊢-family constructor implicits
--     unsolvable).  The facade keeps one copy downstream by re-exporting
--     the Defs slice from the Base facade.

open import S4dot2.TokenAlgebra.Interface

module S4dot2.Generic.CutElimination.Base (TA : TokenAlgebra) where
...
open import S4dot2.Generic.System TA hiding (discretePFormula)
open import S4dot2.Generic.System TA using (discretePFormula) public

-- Re-export lightweight definitions (degree, height, δ, isCutFree, leq-max-*, etc.)
open import S4dot2.Generic.CutElimination.Defs TA public
