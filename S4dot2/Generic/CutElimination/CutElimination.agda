...
-- S4dot2/CutElimination/CutElimination.agda lines 22-313; only this header
-- differs:
--   * module parameterized over TA; Syntax/System/Base/MixNew opens flipped
--     to the Generic layer (ListExt opened directly; the Defs surface
--     arrives through the generic Base's PUBLIC Defs re-export -- exactly
--     as today's file takes it through Base -- sharing Base's single
--     module-application copy of the measure heads);
--   * a private local module tree S4dot2.CutElimination.Base aliases the
--     generic Base's mem-removeAll-subset / not-in-removeAll so the body's
--     qualified references resolve verbatim (qualified as-names are
--     rejected by Agda 2.9).

open import S4dot2.TokenAlgebra.Interface

module S4dot2.Generic.CutElimination.CutElimination (TA : TokenAlgebra) where
...
open import S4dot2.Generic.Syntax TA hiding (_⊢_)
open import S4dot2.Generic.System TA
open import S4dot2.Generic.CutElimination.Base TA hiding (<-wf)
open import S4dot2.Generic.CutElimination.MixNew TA using (mix)

-- Header device: local module tree giving the body's qualified
-- S4dot2.CutElimination.Base.* references their referents
...
private
  ...
  module S4dot2 where
    module CutElimination where
      module Base where
        open GenericBaseApplied using (mem-removeAll-subset; not-in-removeAll) public
