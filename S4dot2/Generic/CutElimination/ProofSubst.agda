...
-- Generic (token-algebra-parameterized) proof-substitution engine
-- (Phase 4 of the token-algebra parameterization).  Bodies ported
-- byte-identically from S4dot2/CutElimination/ProofSubst.agda except:
--   * imports: facade opens replaced by Generic.{Syntax,System} TA and
--     the Generic/PositionCompat layer (SortedPosition spellings);
--   * Tier-B re-points: remove-singleton-self / ∈Pos-singleton /
--     ∉Pos-singleton now come from the interface fields (their bodies
--     pattern-matched on the SDL sum shape of ∈Pos);
--   * Tier-C rebase (bound engine): maxTokenPos := listMaxℕ ∘ map tokℕ ∘
--     toList (replaces the SDL-head trick); >-maxTokenPos-∉Pos and
--     x≤maxTokenPos-insertToken re-proved from the listability fields;
--     freshTokenForRename / freshTokenCtx mint via freshAbove; the
--     MWFResult counter lives in measure ℕ, with each eigen node minting
--     x' = freshAbove next and bumping to suc (tokℕ x'); every eigen
--     bound "maxEigenposToken Π < x" becomes "... < tokℕ x".
--   Under the NAT-SDL bindings (tokℕ = id, freshAbove = suc) every
--   exported type elaborates to today's type (gate-3 pin:
--   S4dot2/TokenAlgebra/Test/ProofSubstAPIPin.agda).
-- The δ/height measure layer stays in the facade until Phase 5
-- (CutElimination.Defs is natSDL-bound and gate-frozen this phase).

open import S4dot2.TokenAlgebra.Interface

module S4dot2.Generic.CutElimination.ProofSubst (TA : TokenAlgebra) where
...
open import Cubical.Data.Nat.Order using (_≤_; _<_; _>_; ≤-refl; zero-≤; suc-≤-suc; pred-≤-pred; ≤-trans; ¬-<-zero; <→≢; <-trans; <-weaken; ¬m<m; ≤<-trans)
...
open import S4dot2.Generic.Syntax TA hiding (_⊢_; singleton-pos) renaming (_∧_ to _and_; _∨_ to _or_)
open import S4dot2.Generic.System TA
open TokenAlgebra TA using (tokℕ; freshAbove; freshAbove-bound; toList; ∈→toList; ∈sing-intro)
open import S4dot2.Generic.CutElimination.PositionCompat TA
  renaming (merge to _++Pos_; [_] to singleton-pos)
...
-- List maximum (measure ℕ): the generic replacement for the SDL-head trick
listMaxℕ : List ℕ → ℕ
listMaxℕ [] = 0
listMaxℕ (n ∷ ns) = myMax n (listMaxℕ ns)

-- Maximum token measure occurring in a position (via the listability fields)
maxTokenPos : Position → ℕ
maxTokenPos s = listMaxℕ (map tokℕ (toList s))
...
freshTokenCtx Γ = freshAbove (maxTokenCtx Γ)
...
remove-singleton-self = remove-sing-self
...
... | no y∉inner = ⊥-rec (y∉inner (mem-++Pos-r {remove x s} {singleton-pos y} (∈sing-intro y)))
... | yes _ =
...
maxEigenposToken (⊢⇒ Π) = maxEigenposToken Π
maxEigenposToken (□⊢ Π) = maxEigenposToken Π
maxEigenposToken (⊢□ {x = x} _ _ _ Π) = myMax (tokℕ x) (maxEigenposToken Π)
maxEigenposToken (♢⊢ {x = x} _ _ _ Π) = myMax (tokℕ x) (maxEigenposToken Π)
maxEigenposToken (⊢♢ Π) = maxEigenposToken Π
...
maxEigenposToken-substTokenToPosProof z t (⊢□ {x} {s} {Γ} {Δ} {A} x∉s xFrΓ xFrΔ Π) (z≢x , ec) (x∉t , nc) =
  ...
  in maxEigenposToken-substR out-path raw
     ∙ cong (myMax (tokℕ x)) (maxEigenposToken-substR (rhs-path ∙ cong (λ p → [ A ^ p ] ,, substCtx z t Δ) pos-eq) Π' ∙ ih)
-- ♢⊢ case: symmetric to ⊢□
maxEigenposToken-substTokenToPosProof z t (♢⊢ {x} {s} {Γ} {Δ} {A} x∉s xFrΓ xFrΔ Π) (z≢x , ec) (x∉t , nc) =
  let Π' = substTokenToPosProof z t Π ec nc
      pos-eq = substPos-insertToken-neq z x s t z≢x
  ...
  in maxEigenposToken-substL out-path raw
     ∙ cong (myMax (tokℕ x)) (maxEigenposToken-substL (lhs-path ∙ cong (λ p → substCtx z t Γ ,, [ A ^ p ]) pos-eq) Π' ∙ ih)
maxEigenposToken-substTokenToPosProof z t (⊢♢ {Γ} {A} {s} {t'} {Δ} Π) ec nc =
  let Π' = substTokenToPosProof z t Π ec nc
...
freshTokenForRename r ctx {Γ} {Δ} Π =
  freshAbove (myMax (maxTokenPos r) (myMax (maxTokenCtx ctx) (myMax (maxTokenCtx Γ) (myMax (maxTokenCtx Δ) (maxEigenposToken Π)))))
...
-- Every token of a list bounds into listMaxℕ (via the tokℕ measure)
∈L-tokℕ-≤ : (y : Token) (xs : List Token) → y ∈L xs → tokℕ y ≤ listMaxℕ (map tokℕ xs)
∈L-tokℕ-≤ y (x ∷ xs) hereL = ≤-myMax-l (tokℕ y) (listMaxℕ (map tokℕ xs))
∈L-tokℕ-≤ y (x ∷ xs) (thereL m) =
  ≤-trans (∈L-tokℕ-≤ y xs m) (≤-myMax-r (tokℕ x) (listMaxℕ (map tokℕ xs)))

-- Membership bounds into maxTokenPos (replaces the SDL-head geometry)
∈Pos-≤-maxTokenPos : ∀ y s → y ∈Pos s → tokℕ y ≤ maxTokenPos s
∈Pos-≤-maxTokenPos y s y∈s = ∈L-tokℕ-≤ y (toList s) (∈→toList y s y∈s)

-- Lemma: x ≤ maxTokenPos (insertToken x r) (re-proved from listability)
x≤maxTokenPos-insertToken : ∀ x r → tokℕ x ≤ maxTokenPos (insertToken x r)
x≤maxTokenPos-insertToken x r =
  ∈Pos-≤-maxTokenPos x (insertToken x r) (insertToken-∈Pos x x r (inl refl))

-- If n is above maxTokenPos s, then n is not in s
>-maxTokenPos-∉Pos : (n : Token) (s : Position) → tokℕ n > maxTokenPos s → n ∉Pos s
>-maxTokenPos-∉Pos n s n>max n∈s = ¬m<m (≤<-trans (∈Pos-≤-maxTokenPos n s n∈s) n>max)
...
>-maxTokenCtx-TokenFresh : (n : Token) (ctx : Ctx) → tokℕ n > maxTokenCtx ctx → TokenFresh n ctx
...
freshTokenForRename-fresh r ctx {Γ} {Δ} Π = >-maxTokenCtx-TokenFresh (freshTokenForRename r ctx Π) ctx n>maxCtx
  where
    ...
    n>maxCtx : tokℕ (freshTokenForRename r ctx Π) > maxTokenCtx ctx
    n>maxCtx = ≤<-trans (≤-trans (≤-myMax-l (maxTokenCtx ctx) _) (≤-myMax-r (maxTokenPos r) inner))
                        (freshAbove-bound (myMax (maxTokenPos r) inner))
...
freshTokenForRename-eigenpos : (r : Position) (ctx : Ctx) {Γ Δ : Ctx} (Π : Γ ⊢ Δ)
  → maxEigenposToken Π < tokℕ (freshTokenForRename r ctx Π)
freshTokenForRename-eigenpos r ctx {Γ} {Δ} Π =
  ≤<-trans (≤-trans step1 (≤-myMax-r (maxTokenPos r) inner))
           (freshAbove-bound (myMax (maxTokenPos r) inner))
  where
    inner = myMax (maxTokenCtx ctx) (myMax (maxTokenCtx Γ) (myMax (maxTokenCtx Δ) (maxEigenposToken Π)))
...
freshTokenForRename-∉r r ctx {Γ} {Δ} Π = >-maxTokenPos-∉Pos (freshTokenForRename r ctx Π) r n>maxr
  where
    inner = myMax (maxTokenCtx ctx) (myMax (maxTokenCtx Γ) (myMax (maxTokenCtx Δ) (maxEigenposToken Π)))
    n>maxr : tokℕ (freshTokenForRename r ctx Π) > maxTokenPos r
    n>maxr = ≤<-trans (≤-myMax-l (maxTokenPos r) inner)
                      (freshAbove-bound (myMax (maxTokenPos r) inner))
...
... | yes x∈t =
  ...
  where
    ...
    comp2 : x' ∈Pos result
    comp2 = mem-++Pos-r {remove x t} {singleton-pos x'} (∈sing-intro x')
...
EigenposCond-from-wf : (x : Token) {Γ Δ : Ctx} (Π : Γ ⊢ Δ) → maxEigenposToken Π < tokℕ x → EigenposCond x Π
...
EigenposCond-from-wf x (⊢□ {x = y} _ _ _ Π) wf =
  ...
  let y<x : tokℕ y < tokℕ x
      y<x = ≤-trans (suc-≤-suc (≤-myMax-l (tokℕ y) _)) wf
      x≢y : x ≢ y
      x≢y x≡y = <→≢ y<x (cong tokℕ (sym x≡y))
  in x≢y , EigenposCond-from-wf x Π (≤-trans (suc-≤-suc (≤-myMax-r (tokℕ y) _)) wf)
EigenposCond-from-wf x (♢⊢ {x = y} _ _ _ Π) wf =
  let y<x = ≤-trans (suc-≤-suc (≤-myMax-l (tokℕ y) _)) wf
      x≢y = λ x≡y → <→≢ y<x (cong tokℕ (sym x≡y))
  in x≢y , EigenposCond-from-wf x Π (≤-trans (suc-≤-suc (≤-myMax-r (tokℕ y) _)) wf)
EigenposCond-from-wf x (⊢♢ Π) wf = EigenposCond-from-wf x Π wf

-- NoEigenposInt for singleton: if all eigentokens < x', then they are ∉ [x']
NoEigenposInt-singleton-fresh : (x' : Token) {Γ Δ : Ctx} (Π : Γ ⊢ Δ)
  → maxEigenposToken Π < tokℕ x' → NoEigenposInt (singleton-pos x') Π
...
NoEigenposInt-singleton-fresh x' (⊢□ {x = y} _ _ _ Π) wf =
  ...
  let y<x' = ≤-trans (suc-≤-suc (≤-myMax-l (tokℕ y) _)) wf
      y≢x' : y ≢ x'
      y≢x' y≡x' = <→≢ y<x' (cong tokℕ y≡x')
      y∉singleton = ∉Pos-singleton y x' y≢x'
  in y∉singleton , NoEigenposInt-singleton-fresh x' Π (≤-trans (suc-≤-suc (≤-myMax-r (tokℕ y) _)) wf)
NoEigenposInt-singleton-fresh x' (♢⊢ {x = y} _ _ _ Π) wf =
  let y<x' = ≤-trans (suc-≤-suc (≤-myMax-l (tokℕ y) _)) wf
      y≢x' = λ y≡x' → <→≢ y<x' (cong tokℕ y≡x')
      y∉singleton = ∉Pos-singleton y x' y≢x'
  in y∉singleton , NoEigenposInt-singleton-fresh x' Π (≤-trans (suc-≤-suc (≤-myMax-r (tokℕ y) _)) wf)
NoEigenposInt-singleton-fresh x' (⊢♢ Π) wf = NoEigenposInt-singleton-fresh x' Π wf
...
renameEigenpos-♢⊢-premise-gen : ∀ {r t : Position} {x : Token} {Γ Δ : Ctx} {A : Formula}
  ...
  → (wf : maxEigenposToken Π < tokℕ x)
  → (x' : Token)
  → (x'-eigenpos : maxEigenposToken Π < tokℕ x')
  → (x'∉r : x' ∉Pos r)
...
NoEigenposInt-from-allGe : (t : Position) (base : ℕ) {Γ Δ : Ctx} (Π : Γ ⊢ Δ)
  → maxTokenPos t < base
  → ((y : Token) → isEigentoken y Π → base ≤ tokℕ y)
  → NoEigenposInt t Π
...
EigenposCond-from-lt-base : (x : Token) (base : ℕ) {Γ Δ : Ctx} (Π : Γ ⊢ Δ)
  → tokℕ x < base
  → ((y : Token) → isEigentoken y Π → base ≤ tokℕ y)  -- all eigentokens ≥ base
  → EigenposCond x Π
...
record MWFResult {Γ Δ : Ctx} (base k : ℕ) : Type where
  ...
  field
    ...
    allEigen≥base : (y : Token) → isEigentoken y proof → base ≤ tokℕ y
...
makeWellFormed-go : (base k : ℕ) → base ≤ k → ∀ {Γ Δ} (Π : Γ ⊢ Δ)
...
makeWellFormed-go base k base≤k (Cut {Γ₁} {A} {s} {Δ₁} {Γ₂} {Δ₂} Π₁ Π₂) origMax<base proofMax<base =
  ...
  where
    ...
    allGe : (y : Token) → isEigentoken y (Cut Π₁' Π₂') → base ≤ tokℕ y
...
makeWellFormed-go base k base≤k (⊢∧ {Γ₁} {A} {s} {Δ₁} {Γ₂} {B} {Δ₂} Π₁ Π₂) origMax<base proofMax<base =
  ...
  where
    ...
    allGe : (y : Token) → isEigentoken y (⊢∧ {Γ₁} {A} {s} {Δ₁} {Γ₂} {B} {Δ₂} (MWFResult.proof mwfR₁) (MWFResult.proof mwfR₂)) → base ≤ tokℕ y
    allGe y (inl yIn₁) = MWFResult.allEigen≥base mwfR₁ y yIn₁
    allGe y (inr yIn₂) = MWFResult.allEigen≥base mwfR₂ y yIn₂
makeWellFormed-go base k base≤k (∨⊢ {Γ₁} {A} {s} {Δ₁} {Γ₂} {B} {Δ₂} Π₁ Π₂) origMax<base proofMax<base =
  mkMWFResult (∨⊢ {Γ₁} {A} {s} {Δ₁} {Γ₂} {B} {Δ₂} (MWFResult.proof mwfR₁) (MWFResult.proof mwfR₂)) (MWFResult.next mwfR₂)
...
makeWellFormed-go base k base≤k (⇒⊢ {Γ₁} {B} {s} {Δ₁} {Γ₂} {A} {Δ₂} Π₁ Π₂) origMax<base proofMax<base =
  ...
  where
    ...
    maxBoth<next' = myMax-<-both max₁<next' (MWFResult.maxEigen<next mwfR₂)
    k≤next' = ≤-trans (MWFResult.k≤next mwfR₁) (MWFResult.k≤next mwfR₂)
    allGe : (y : Token) → isEigentoken y (⇒⊢ {Γ₁} {B} {s} {Δ₁} {Γ₂} {A} {Δ₂} (MWFResult.proof mwfR₁) (MWFResult.proof mwfR₂)) → base ≤ tokℕ y
    allGe y (inl yIn₁) = MWFResult.allEigen≥base mwfR₁ y yIn₁
    allGe y (inr yIn₂) = MWFResult.allEigen≥base mwfR₂ y yIn₂
...
makeWellFormed-go base k base≤k (⊢□ {x} {r} {Γ} {Δ} {A} x∉r xFrΓ xFrΔ Π) origMax<base proofMax<base = result
  where
    ...
    x<base : tokℕ x < base
    x<base = ≤-trans (suc-≤-suc (≤-myMax-l (tokℕ x) _)) origMax<base

    subMax<base : maxEigenposToken Π < base
    subMax<base = ≤-trans (suc-≤-suc (≤-myMax-r (tokℕ x) _)) origMax<base
    ...
    k≤x' : k ≤ tokℕ x'
    k≤x' = ≤-trans (MWFResult.k≤next mwfR) (<-weaken (freshAbove-bound next))
    ...
    nc = NoEigenposInt-singleton-fresh x' Π-wf (≤-trans (MWFResult.maxEigen<next mwfR) (<-weaken (freshAbove-bound next)))
    ...
    x'>r : tokℕ x' > maxTokenPos r
    x'>r = ≤-trans r<base (≤-trans base≤k k≤x')

    x'>Γ : tokℕ x' > maxTokenCtx Γ
    x'>Γ = ≤-trans Γ<base (≤-trans base≤k k≤x')

    x'>Δ : tokℕ x' > maxTokenCtx Δ
    ...
    next≤next' : next ≤ next'
    next≤next' = ≤-trans (<-weaken (freshAbove-bound next)) (≤-suc (tokℕ x'))
    ...
    allGeNew : (y : Token) → isEigentoken y result-proof → base ≤ tokℕ y
    allGeNew y (inl y≡x') = subst (λ w → base ≤ tokℕ w) (sym y≡x') base≤x'
    allGeNew y (inr yInΠ') =
...
makeWellFormed-base : ∀ {Γ Δ} → (Π : Γ ⊢ Δ) → ℕ
...
WellFormedProof (⊢⇒ Π) = WellFormedProof Π
WellFormedProof (□⊢ Π) = WellFormedProof Π
WellFormedProof (⊢□ {x = x} _ _ _ Π) = (maxEigenposToken Π < tokℕ x) × WellFormedProof Π
WellFormedProof (♢⊢ {x = x} _ _ _ Π) = (maxEigenposToken Π < tokℕ x) × WellFormedProof Π
WellFormedProof (⊢♢ Π) = WellFormedProof Π
...
WellFormed-substTokenToPosProof z t (⊢□ {x} {s} {Γ} {Δ} {A} x∉s xFrΓ xFrΔ Π) (z≢x , ec) (x∉t , nc) (maxΠ<x , wf) =
  let Π' = substTokenToPosProof z t Π ec nc
      ...
      maxΠ'<x : maxEigenposToken Π' < tokℕ x
      maxΠ'<x = subst (_< tokℕ x) (sym maxΠ'-eq) maxΠ<x
...
makeWellFormed-go-WellFormed : (base k : ℕ) (base≤k : base ≤ k) {Γ Δ : Ctx} (Π : Γ ⊢ Δ)
  → (origMax<base : maxEigenposToken Π < base)
  → (proofMax<base : maxTokenProof Π < base)
...
makeWellFormed-go-WellFormed base k base≤k (⊢□ {x} {r} {Γ} {Δ} {A} x∉r xFrΓ xFrΔ Π) origMax<base proofMax<base =
  ...
  where
    ...
    wf-bound : maxEigenposToken Π' < tokℕ x'
    wf-bound = subst (_< tokℕ x') (sym maxΠ'-eq) (≤-trans (MWFResult.maxEigen<next mwfR) (<-weaken (freshAbove-bound next)))
...
-- =============================================================================
-- δ/height measure layer (Phase 5 of the token-algebra parameterization):
-- moved here from the facade (S4dot2/CutElimination/ProofSubst.agda:54-1003);
-- text is the facade's verbatim except the generic bound-engine spellings
-- (maxEigenposToken _ < tokℕ _, x' = freshAbove next, the freshAbove-bound
-- wrapper) — same fidelity discipline as the Tier-C engine above (at
-- tokℕ = id / freshAbove = suc every exported type elaborates to today's).
-- =============================================================================

open import Cubical.Data.Nat using (max; _+_)
open import S4dot2.Generic.CutElimination.Defs TA using (δ; degree; height)
...
δ-makeWellFormed-go : (base k : ℕ) (base≤k : base ≤ k) {Γ Δ : Ctx} (Π : Γ ⊢ Δ)
  → (origMax<base : maxEigenposToken Π < base)
  → (proofMax<base : maxTokenProof Π < base)
...
height-makeWellFormed-go : (base k : ℕ) (base≤k : base ≤ k) {Γ Δ : Ctx}
  → (Π : Γ ⊢ Δ)
  → (origMax<base : maxEigenposToken Π < base)
  → (proofMax<base : maxTokenProof Π < base)
...
substEigenposInProof x t {Γ} {Δ} Π wf = (Π-subst , h-pres , δ-pres)
  where
    ...
    innerMax2 = myMax (tokℕ x) (maxTokenPos t)
    ...
    x<base : tokℕ x < base
    x<base = suc-≤-suc (≤-trans (≤-myMax-l (tokℕ x) (maxTokenPos t)) (≤-myMax-r innerMax1 innerMax2))

    t<base : maxTokenPos t < base
    t<base = suc-≤-suc (≤-trans (≤-myMax-r (tokℕ x) (maxTokenPos t)) (≤-myMax-r innerMax1 innerMax2))
