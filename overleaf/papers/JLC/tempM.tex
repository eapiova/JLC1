...
record TokenAlgebra : Type1 where
  field
    ...
    sing            : Token -> Pos
    _cup_           : Pos -> Pos -> Pos
    insert remove   : Token -> Pos -> Pos
    _in_            : Token -> Pos -> Type0
    _in?_           : forall x s -> Dec (x in s)

    -- algebraic, membership, and removal laws omitted
    posExt           : ...
    toList           : Pos -> List Token
    tokNat           : Token -> Nat
    freshAbove       : Nat -> Token
    freshAbove-bound : forall n -> n < tokNat (freshAbove n)
\end{verbatim}

\noindent
(The display renders the Unicode identifiers of the source in ASCII; the interface itself is
linked above.)

The public development instantiates it with natural-number tokens and sorted descending lists. As a non-trivial parametricity check, the repository also instantiates the full generic Mix and cut-elimination modules with tagged tokens $\mathbb{N}\times\mathsf{Bool}$\agdalink{S4dot2.TokenAlgebra.Instances.TaggedToken.html\#taggedToken}, ordered through the injective measure $(n,b)\mapsto 2n+\mathsf{bit}(b)$. This second instance still represents positions by SDLs: it establishes independence from the concrete token carrier, not yet from the position data structure itself. A normalised trie could in principle provide another implementation of the same interface, but we have neither implemented nor benchmarked such an alternative.

This abstraction boundary currently covers syntax and the proof-theoretic cut-elimination development. The semantic, Hilbert-equivalence, and completeness developments still use the concrete SDL-based public modules.
...
%% NOTA REVISIONE (decisione pendente, R1-6a-quater): qui va deciso se aggiungere una nota
%% di erratum che segnali la correzione rispetto alle presentazioni precedenti del gruppo,
%% con rinvio al testimone S4dot2/Algebra/LegacyBasisRefuted.agda.
...
The system $\eseq$ proves, at the empty position, all the theorems of the Hilbert-style presentation of $\logic$ (see also~\cite{Martini2024:S42NaturalDeduction}). The derivations below are given at the empty position $\varnothing$. Several of them, namely the propositional schemas together with $\axname{D}$ and $\axname{T}$, are in fact established at an arbitrary position, and $\varnothing$ is only the instance used here. Transporting a derivation from $\varnothing$ to an arbitrary position $s$ is not a matter of weakening, since $W\vdash$ and $\vdash W$ add a p-formula to a context and leave positions untouched: the mechanism is the position lifting lemma, which raises every position occurring in a derivation and carries a freshness proviso on the eigenpositions.
...
examples\agdalink{S4dot2.CutElimination.Examples.html\#Examples} records four
...
\begin{corollary}[Subformula Property \agdalink{S4dot2.CutElimination.SubformulaProperty.html\#SubformulaProperty\%27}]\label{cor:subformula}
...
We conclude the section by showing the \agda{} type of the Subformula Property, i.e.\ of Corollary~\ref{cor:subformula}:
\[
\begin{array}{l}
\text{\texttt{SubformulaProperty'}} : \{\Gamma\;\Delta : \text{\texttt{Ctx}}\} \to (\Pi : \seq{\Gamma}{\Delta}) \to \text{\texttt{isCutFree}}\;\Pi \\
\qquad {}\to (\text{\texttt{pf}} : \text{\texttt{PFormula}}) \to \text{\texttt{pf}} \in \text{\texttt{allFormulasInProof}}\;\Pi \to \text{\texttt{isSubformulaOfSeq}}\;\text{\texttt{pf}}\;\Gamma\;\Delta
\end{array}
\]

\noindent
The function takes a cut-free proof $\Pi$ (witnessed by \texttt{isCutFree}), a p-formula \texttt{pf} occurring anywhere in $\Pi$ --- \texttt{allFormulasInProof}\agdalink{S4dot2.CutElimination.SubformulaProperty.html\#allFormulasInProof} collects the antecedent and the succedent of every sequent of the derivation --- and returns a proof that \texttt{pf} is a subformula of some formula in the end-sequent.
It is defined by induction on the derivation: the base case (\textsc{Ax}) is immediate; the Cut case is discharged by contradiction, since a proof with $\delta = 0$ cannot end in a Cut rule; for every other rule the property is propagated from the premises by the induction hypothesis, the four modal rules using that a position is contained in each of its extensions, which is exactly the position extension allowed by Definition~\ref{def:subform}.

The development also contains a weaker variant, \texttt{SubformulaProperty}\agdalink{S4dot2.CutElimination.SubformulaProperty.html\#SubformulaProperty}, whose statement quantifies over \texttt{formulasInProof}\agdalink{S4dot2.CutElimination.SubformulaProperty.html\#formulasInProof} rather than over \texttt{allFormulasInProof}. Since \texttt{formulasInProof} collects only the cut formulae of a derivation, and is therefore provably empty on cut-free ones, that variant is vacuously true and carries no information; the statement displayed above is the faithful one, and it is the one Corollary~\ref{cor:subformula} refers to.
...
\begin{definition}[Segerberg FMP \agdalink{S4dot2.Equivalence.ClassicalEquivalence.html\#Classical}]\label{post:segerberg-fmp}
...
\begin{definition}[Decidability of $\vdash_{\hilbertlogic{\logic}}$ \agdalink{S4dot2.Equivalence.ClassicalEquivalence.html\#Classical}]\label{post:decidable}
...
This project constitutes a closely related reference, as our proposed implementation is part of an ongoing effort towards a modular implementation covering the spectrum from $\mathsf{K}$ to $\axname{S}5$ (see Section~\ref{sec:conclusions_future}).
...
Finally, the results presented in this work are part of a broader research project whose core is a unified framework, denoted by $\framework$ (\textsf{New Agda Modal Realization}), implemented in Cubical \agda{} and building upon the theory of extended sequents~\cite{Martini2023:CutElimination}. The aim of $\framework$ is to encompass the full range of normal modal logics from $\mathsf{K}$ to $\mathsf{S5}$, including $\logic$. In accordance with the underlying theoretical systems, $\framework$ is designed to be highly parametric, since all normal modal systems share a common set of rules and individual systems can be obtained by appropriately instantiating specific parameters. The current $\framework$ cut-elimination construction is uniform for the insensitive presentations that instantiate its full coherence and Mix interface. The crucial eigen-root substitution law has also been verified for the prefix relations used by $\axname{K}$, $\axname{K4}$, and $\axname{S4}$, and trivially for $\axname{S5}$; this local coherence result is not, however, a cut-elimination theorem for the sensitive $\axname{K}$ and $\axname{K4}$ presentations. $\logic$ shares $\framework$'s mode-theory, constraint, and adequacy infrastructure, while its current cut-elimination result inside $\framework$ is obtained by verified replay through the standalone $\logic$ calculus. Machine-checked counterexamples delimit the present eigen-root rebase interface for $\logic$, $\axname{S4.3}$, and $\axname{S4.4}$; they do not refute cut elimination for those logics. Uniform coverage from $\axname{K}$ to $\axname{S5}$ remains future work. In light of the considerations discussed above, we are also investigating possible extensions of $\framework$ to non-normal modal logics.
