...
The system $\eseq$ proves, at any position, all the theorems of the Hilbert-style presentation of $\logic$ (see also~\cite{Martini2024:S42NaturalDeduction}). The derivations below are given at the empty position $\varnothing$. Several of them, namely the propositional schemas together with $\axname{D}$ and $\axname{T}$, are in fact established at an arbitrary position, and $\varnothing$ is only the instance used here. Transporting a derivation from $\varnothing$ to an arbitrary position $s$ is not a matter of weakening, since $W\vdash$ and $\vdash W$ add a p-formula to a context and leave positions untouched: the mechanism is the position lifting lemma, which raises every position occurring in a derivation and carries a freshness proviso on the eigenpositions.
