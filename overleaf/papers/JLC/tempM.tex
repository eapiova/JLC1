...
\newcommand{\agdaBaseUrl}{https://eapiova.github.io/JLC1}  % Agda HTML hosted via GitHub Pages
...
% Evidenziatore giallo, stessa logica: neutro qui, acceso nel diff (lua-ul).
\usepackage{luacolor}
\usepackage{lua-ul}
\newcommand{\spotlighty}[1]{#1}
...
\begin{abstract}
    ...
    This paper is an extended version of~\cite{Borsetto2025:S42Investigations}, presented at ICTCS 2025. That contribution already announced the formalisation of cut elimination and of its corollaries, and discussed weak completeness with respect to the Hilbert-style axiomatisation; the semantics was only sketched there. With respect to it, the present version contributes a complete cut-elimination development, the finite semilattice semantics with a decidable evaluation and a full soundness proof, the direction from the sequent calculus back to the Hilbert system, and a precise account of which ingredients of the equivalence remain classical.
\end{abstract}
...
\textbf{Guide to the reading.} The complete \agda{} development is available at \url{https://github.com/eapiova/JLC1}, together with a browsable, hyperlinked HTML rendering of every module at \url{https://eapiova.github.io/JLC1}. The reader can access the code through the symbol ``\agdalink{}''. When particularly meaningful, we also include and comment on excerpts of the code along the paper.
...
\spotlighty{Since the development is constructive, the elimination procedure is not only a
...
\spotlighty{The dividing line follows the analysis of the principal case given above: the two
...
\begin{theorem}[Subformula Property \agdalink{S4dot2.CutElimination.SubformulaProperty.html\#SubformulaProperty}]\label{cor:subformula}
...
The referee-visible boundary of this section deserves to be stated exactly. The two module parameters play different roles. The Segerberg FMP is a semantic completeness input, and its justification above is a classical argument about filtrations; the decidability of $\vdash_{\hilbertlogic{\logic}}$ is what classically \emph{follows} from the FMP, by an argument due to Harrop: interleave an enumeration of the Hilbert derivations with an enumeration of the finite models by increasing size; by the FMP, for every $A$ one of the two searches succeeds. The argument is classical twice over: knowing that one of the searches succeeds does not compute which, and a failed finite search for a countermodel constructively refutes only the refutability of $A$, not its underivability. Turning the implication into a theorem of the development would require three ingredients that the formalisation does not currently have, together with a choice of form for the first. First, the FMP in \emph{bounded} form, dispensing with the enumeration of derivations: the filtration argument bounds the size of the countermodel by an exponential in the number of subformulae of $A$, but the parameter as stated merely asserts the existence of \emph{some} finite model, so a search has no bound to iterate to; and to conclude derivability rather than its double negation, the bounded input must be taken \emph{positively}, as validity in all models up to the bound implying derivability. Second, an effective enumeration of the finite models up to that bound. This is more than the finiteness of the set of worlds: a finite model here is a record carrying the order, the minimum, the join operation and the proofs of its laws, so an enumeration needs codes for all of these together with a decidable verification of the laws, and the valuation field ranges over all atoms, so a finite-support lemma is needed to cut the space down to a finite one. Third, with the first two in place, validity in all models up to the bound becomes decidable, and the decidability of $\vdash_{\hilbertlogic{\logic}}$ follows, reducing the classical inputs from two to one, namely the positively stated bounded FMP; as noted in the justification above, even the passage from the filtrated frame to a join-semilattice requires an argument we have not given. We regard this internalisation as future work and make no claim about its difficulty.
...
Our investigation opens several promising research directions. As ongoing work, we are focusing on the definition of an intuitionistic version of $\logic$, denoted by $\mathsf{I}\logic$, which constitutes a non-trivial task. On the one hand, a natural candidate for the intuitionistic deductive system is obtained by restricting sequents to have a single formula on the right-hand side. On the other hand, a standard Hilbert-style intuitionistic axiomatisation is (up to our knowledge) not available in the literature and must therefore be carefully developed, particularly with regard to the elimination of classical axioms.
Moreover, extending the approach partially adopted in this paper for finite model semantics, we aim to develop a fully intuitionistic metatheory and semantics for $\mathsf{I}\logic$, based exclusively on constructive mathematical principles.
In addition to the intrinsic interest in intuitionistic logic, we plan to investigate the Curry–Howard correspondence for $\mathsf{I}\logic$. Understanding the computational content of $\logic$ appears especially attractive, given its wide range of applications and the characteristic properties of its models, which capture strong features of rewriting systems and programs.
